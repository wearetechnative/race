# Specification: Git Sync After Apply

## Overview

This specification defines the git synchronization capability that automatically commits, tags, and pushes changes after successful infrastructure apply operations.
...
- Related to: Environment detection from `.terraform/tfbackend.state`
- Related to: `_racelib.sh` shared function library
- Related to: `tfapply.sh` terraform apply wrapper
- Related to: `nixrun.sh` nix run wrapper
